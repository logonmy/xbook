...
import (
	...
	"github.com/ziyoubiancheng/xbook/utils/html2md"
)
...
//diySelecter:自定义选择器
//注意：由于参数问题，采集并下载图片的话，在headers中加上key为"project"的字段，值为文档图书的标识
func CrawlHtml2Markdown(urlstr string, contType int, force bool, intelligence int, diySelector string, excludeSelector []string, links map[string]string, headers ...map[string]string) (cont string, err error) {
	...
	if force { //强力模式
		...
		req := common.BuildRequest("get", urlstr, "", "", "", true, false, headers...)
...
func DownImage(src string, headers ...map[string]string) (filename string, err error) {
	...
	//url链接图片
	resp, err = common.BuildRequest("get", src, src, "", "", true, false, headers...).Response()
	if err != nil {
		return
	}
	defer resp.Body.Close()
...
func MD5Sub16(str string) string {
	return common.Md5Crypt(str)[0:16]
}
