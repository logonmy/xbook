...
import (
	...
	"github.com/ziyoubiancheng/xbook/utils/graphics"
)
...
func (this *BookController) unzipToData(bookId int, identify, zipFile, originFilename string) {
	...
	if err := common.Unzip(zipFile, unzipPath); err != nil {
		beego.Error("解压失败", zipFile, err.Error())
	} else {

		//读取文件，把图片文档录入oss
		if files, err := common.ScanFiles(unzipPath); err == nil {
...
func (this *BookController) loadByFolder(bookId int, identify, folder string) {
	...
	//读取文件，把图片文档录入oss
	files, err := common.ScanFiles(folder)
	if err != nil {
		beego.Error(err)
		return
	}
...
func (this *BookController) replaceToAbs(projectRoot string, identify string) {
	...
	files, _ := common.ScanFiles(projectRoot)
	for _, file := range files {
