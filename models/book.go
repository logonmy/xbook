...
import (
	...
	"github.com/ziyoubiancheng/xbook/models/store"
...
type Book struct {
	...
	BookName          string    `orm:"column(book_name);size(500)" json:"book_name"`      // BookName 图书名称.
	Identify          string    `orm:"column(identify);size(100);unique" json:"identify"` // Identify 图书唯一标识.
	...
	Description       string    `orm:"column(description);size(2000)" json:"description"` // Description 图书描述.
	Label             string    `orm:"column(label);size(500)" json:"label"`
	PrivatelyOwned    int       `orm:"column(privately_owned);type(int);default(0)" json:"privately_owned"` // PrivatelyOwned 图书私有： 0 公开/ 1 私有
	PrivateToken      string    `orm:"column(private_token);size(500);null" json:"private_token"`           // 当图书是私有时的访问Token.
	...
	ReleaseTime       time.Time `orm:"type(datetime);column(release_time);" json:"release_time"`   //图书发布时间，每次发布都更新一次，如果文档更新时间小于发布时间，则文档不再执行发布
	...
	Vcnt              int       `orm:"column(vcnt);default(0)" json:"vcnt"`    //文档图书被阅读次数
	Star              int       `orm:"column(star);default(0)" json:"star"`    //文档图书被收藏次数
	Score             int       `orm:"column(score);default(40)" json:"score"` //文档图书评分，默认40，即4.0星
	CntScore          int       //评分人数
	CntComment        int       //评论人数
